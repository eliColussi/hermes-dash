  ...
  return (
    <div className="card p-4 mb-4 border-accent/40 bg-accent-soft/40">
      ...
      <p className="text-[11px] text-muted mt-3">
        ...
        pairing code in their first message and has to wait here. Already
        handled them via a newer code? Hit the X to dismiss.
      </p>
    </div>
  );
}
...
function PendingRow({
  ...
  const dismiss = useMutation({
    mutationFn: () => api.dismissPairing(p.platform, p.code),
    onSuccess: () => {
      setDone("dismissed");
  ...
  const err = (approve.error || dismiss.error) as Error | null;
      ...
      {done === "approved" ? (
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            disabled={busy}
            className="p-1.5 rounded text-muted hover:bg-black/5 dark:hover:bg-white/10 disabled:opacity-50"
            title="Dismiss this code without approving (it'll disappear from the banner)"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
